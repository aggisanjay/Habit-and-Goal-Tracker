...
router.get('/', protect, async (req, res) => {
  try {
    ...
    const goals = await Goal.find(filter)
      .populate('linkedHabits', 'name icon color')
      .sort({ createdAt: -1 });
    // Strip nulls from populate (happens when a linked habit was deleted)
    const safe = goals.map(g => {
      const obj = g.toObject();
      obj.linkedHabits = (obj.linkedHabits || []).filter(Boolean);
      return obj;
    });
    res.json({ success: true, count: safe.length, goals: safe });
  } catch (err) {
...
router.get('/:id', protect, async (req, res) => {
  try {
    ...
    console.error('[GET /goals/:id]', err.message);
    res.status(500).json({ success: false, message: err.message });
  }
});
...
// PUT /api/goals/:id/subtasks/:taskId - Toggle subtask
router.put('/:id/subtasks/:taskId', protect, async (req, res) => {
  try {
    const goal = await Goal.findOne({ _id: req.params.id, user: req.user._id });
    if (!goal) return res.status(404).json({ success: false, message: 'Goal not found' });
